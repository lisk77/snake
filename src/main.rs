...
fn update_timers(app: &mut App, dt: f32) {
    app.query::<&mut Timer, ()>().for_each(|t| {
        t.update_timer(dt);
    });
}
...
fn update_snake(app: &mut App) {
    ...
    app.query::<&mut Timer, ()>().iter().next().unwrap().reset();
}
...
fn update_snake_direction(app: &mut App) {
    ...
    let head_dir = app.query::<&mut Direction, ()>().iter().next().unwrap();
    head_dir.update();

    for (i, d) in app.query::<&mut Direction, ()>().iter().skip(1).enumerate() {
        d.set_direction(directions[i]);
    }
}

fn update_snake_orientation(app: &mut App) {
    app.query::<(&mut Transform2D, &Direction), ()>()
        .for_each(|(t, d)| {
            let dir = d.direction();
...
fn update_snake_textures(app: &mut App) {
    ...
    for (i, (r, t)) in app
        .query::<(&mut Render2D, &mut Transform2D), With<Snake>>()
        .iter()
        .enumerate()
    {
        if i == 0 {
...
fn update_snake_position(app: &mut App) {
    ...
    for (i, (t, d)) in app
        .query::<(&mut Transform2D, &Direction), With<Snake>>()
        .iter()
        .enumerate()
    {
...
fn update_snake_colliders(app: &mut App) {
    ...
    for (i, (r, d)) in app
        .query::<(&mut Rectangle2D, &Direction), With<Snake>>()
        .iter()
        .enumerate()
    {
...
fn snake_out_of_bounds(app: &mut App) -> bool {
    let field_collider = app
        .query::<(&Rectangle2D, &Grid), ()>()
        .iter()
        .next()
        .unwrap()
        .0
...
fn handle_apple_collision(app: &mut App) {
    ...
    let apple_collider = app
        .query::<&Rectangle2D, With<Apple>>()
        .iter()
        .next()
        .unwrap()
        .clone();
    ...
    snake_head_collider.is_colliding(&apple_collider).then(|| {
        app.play_audio("nom", false);
        app.set_volume("nom", 0.5);

        move_apple(app);

        let (tail_transform, tail_dir) = app
            .query::<(&Transform2D, &Direction), With<Snake>>()
            .iter()
            .last()
            .unwrap();
